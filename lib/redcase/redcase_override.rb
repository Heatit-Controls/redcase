  ...
  module RedcaseOverride
    ...
    class RO < Redmine::Hook::ViewListener
      ...
      def controller_issues_edit_after_save(context = { })
        journal_details = JournalDetail.find_by(journal_id: context[:journal])
        if journal_details.nil? then
          # ???
          return
        end
        if journal_details.prop_key == 'tracker_id' then
          if journal_details.value == Tracker.find_by(name: 'Test case').id.to_s then
            controller_issues_new_after_save(context)
          elsif journal_details.old_value == Tracker.find_by(name: 'Test case').id.to_s then
