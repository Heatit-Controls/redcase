...
module Redcase::Rtf::Converters
  class HTML
    ...
    def to_rtf_document(options = {})
      ...
      Redcase::Rtf::Document.new(font).tap do |rtf|
        nodes.to_rtf(rtf)
      end
    end
    ...
    module Helpers
      ...
      def font(key)
        Redcase::Rtf::Font.new(*case key
          when :default   then [Redcase::Rtf::Font::ROMAN,  'Times New Roman']
          when :monospace then [Redcase::Rtf::Font::MODERN, 'Courier New'    ]
        end)
      end

      def style(key)
        Redcase::Rtf::CharacterStyle.new.tap do |style|
          case key.to_sym
          when :h1
            style.font_size = 44
            style.bold = true
